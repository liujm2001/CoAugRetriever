...
class ActorRolloutRefWorker(Worker):
    ...
    def save_checkpoint(self, local_path, hdfs_path=None):
        ...
        if self.rank == 0:
            ...
            self.actor_module.save_pretrained(local_path, state_dict=state_dict, torch_dtype=torch.bfloat16)
            self.tokenizer.save_pretrained(local_path, torch_dtype=torch.bfloat16)
...
class RewardModelWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def init_dense_model(self, embedmodel_path):
        from FlagEmbedding import FlagAutoModel
        from transformers import AutoModelForTokenClassification, AutoConfig
        from torch.distributed.fsdp import FullyShardedDataParallel as FSDP, ShardingStrategy, CPUOffload

        import ray
        # print(f"Process {os.getpid()} loading dense_model")
        # print('load', ray.get_gpu_ids(), os.environ["CUDA_VISIBLE_DEVICES"])

        dense_model = FlagAutoModel.from_finetuned(
            embedmodel_path,
            query_instruction_for_retrieval="Represent this sentence for searching relevant passages:",
            torch_dtype="auto",
            devices=[f"cuda:{ray.get_gpu_ids()[0]}"]
        )

        self.dense_model = dense_model

        # torch.cuda.set_device(ray.get_gpu_ids()[0])
        # print(f"Process {os.getpid()} inferring dense_model")
        # print('infer', ray.get_gpu_ids(), os.environ["CUDA_VISIBLE_DEVICES"], torch.cuda.current_device(), flush=True)
        torch.cuda.empty_cache()

    @register(dispatch_mode=Dispatch.DP_COMPUTE_DENSE_PROTO)
    def dense_model_encode(self, data):
        # import ray
        # print(f"Process {os.getpid()} inferring dense_model")
        # print('infer', ray.get_gpu_ids(), os.environ["CUDA_VISIBLE_DEVICES"])
        
        return self.dense_model.encode_single_device(data, device=f"cuda:0")  
