...
def dispatch_dp_compute_data_dense_proto(worker_group, *args, **kwargs):
    sharded_args = []
    for arg in args:
        shard_size = len(arg) // worker_group.world_size
        sharded_args.append([arg[i*shard_size : (i+1)*shard_size] for i in range(worker_group.world_size)])
    sharded_kwargs = {}
    for k, v in kwargs.items():
        shard_size = len(v) // worker_group.world_size
        sharded_kwargs[k] = [v[i*shard_size : (i+1)*shard_size] for i in range(worker_group.world_size)]
...
def collect_dp_compute_data_dense_proto(worker_group, output):
    return [item for shard in output for item in shard]
...
def get_predefined_dispatch_fn(dispatch_mode):
    predefined_dispatch_mode_fn = {
        ...
        Dispatch.DP_COMPUTE_DENSE_PROTO: {
            'dispatch_fn': dispatch_dp_compute_data_dense_proto,
            'collect_fn': collect_dp_compute_data_dense_proto
        }
    }
